...
export default class CreateSnippetView extends BaseView {
  ...
  submitForm() {
    ...
    let selectLibraryVal = this.$el.find('.select-library').val();
    let libraryGuid;

    if (selectLibraryVal === 'personal') {
      libraryGuid = store.personalLibrary.guid;
    } else {
      libraryGuid = selectLibraryVal;
    }

    let description = this.$el.find('.input-description').val().trim();

    let labelGuids = [];
    let labelGuid = this.$el.find('.select-label').val();
    if (labelGuid) {
